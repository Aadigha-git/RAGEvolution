#!/usr/bin/env python3
"""
Chunk GLP-1 FDA label text, embed with Gemini, and load into Supabase.

Version 1.0 naive RAG: fixed-size character chunking (1000 / 200 overlap),
gemini-embedding-001 vectors (3072 dimensions) stored in halfvec(3072).
"""
...
SUPABASE_EMBEDDING_COLUMN = "halfvec(3072)"
SECTION_FIELDS = (
...
PIPELINE_METADATA = {"pipeline_version": "v1.0_naive"}
...
def setup_logging() -> logging.Logger:
    logger = logging.getLogger("populate_vector_db")
...
def init_clients() -> tuple[Client, genai.Client]:
    ...
    # google-genai reads GOOGLE_API_KEY when Client() is called without arguments.
...
def iter_chunk_records(records: list[dict[str, Any]]) -> Iterator[ChunkRecord]:
    for record_index, record in enumerate(records):
        ...
        for section_name in SECTION_FIELDS:
            ...
            for chunk_index, chunk_content in enumerate(
                chunk_text(section_text, CHUNK_SIZE, CHUNK_OVERLAP)
            ):
                yield ChunkRecord(
...
def build_row(chunk: ChunkRecord, embedding: list[float]) -> dict[str, Any]:
    return {
...
def populate(
    ...
    all_chunks = list(iter_chunk_records(records))
    total_chunks = len(all_chunks)

    logger.info("Loaded %d drug records from %s", len(records), input_path)
    logger.info(
        "Prepared %d text chunks (%d chars, %d overlap) across sections: %s",
        total_chunks,
        CHUNK_SIZE,
        CHUNK_OVERLAP,
        ", ".join(SECTION_FIELDS),
    ...
    if total_chunks == 0:
        logger.warning("No chunks generated; nothing to insert.")
        return 0
    ...
    if dry_run:
        ...
        for idx, chunk in enumerate(all_chunks[:5], start=1):
            logger.info(
                "[%d/%d] sample chunk | brand=%s | section=%s | chars=%d",
                idx,
                total_chunks,
                chunk.brand_name,
                chunk.section_name,
    ...
    logger.info(
        "Gemini model: %s | Supabase target: embedding %s | expected output: %d dimensions",
        EMBEDDING_MODEL,
        SUPABASE_EMBEDDING_COLUMN,
    ...
    for batch_start in range(0, total_chunks, embed_batch_size):
        ...
        for offset, chunk in enumerate(batch):
            ...
            try:
                ...
                logger.info(
                    "[%d/%d] embedded %d-dim vector → %s | brand=%s | generic=%s | section=%s | chunk=%d",
                    global_index,
                    total_chunks,
                    len(embedding),
                    SUPABASE_EMBEDDING_COLUMN,
                ...
                logger.error(
                    "[%d/%d] embedding failed | brand=%s | section=%s | error=%s",
                    global_index,
                    total_chunks,
                    chunk.brand_name,
                    chunk.section_name,
            ...
            if len(pending_rows) >= insert_batch_size:
                insert_batch(supabase, pending_rows, logger)
                inserted += len(pending_rows)
                logger.info(
                    "Inserted batch of %d rows (%d-dim → %s) | running total: %d/%d",
                    len(pending_rows),
                    len(pending_rows[0]["embedding"]),
                    SUPABASE_EMBEDDING_COLUMN,
                    inserted,
                    total_chunks,
                )
    ...
    if pending_rows:
        ...
        inserted += len(pending_rows)
        logger.info(
            "Inserted final batch of %d rows (%d-dim → %s) | running total: %d/%d",
            len(pending_rows),
            len(pending_rows[0]["embedding"]),
            SUPABASE_EMBEDDING_COLUMN,
            inserted,
    ...
    logger.info(
        "Population complete: %d rows inserted into fda_document_chunks.embedding %s",
        inserted,
        SUPABASE_EMBEDDING_COLUMN,
    )
    return inserted
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Chunk GLP-1 FDA labels, embed with Gemini, load into Supabase."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Chunks to embed per logging batch",
    )
    parser.add_argument(
        "--insert-batch-size",
        type=int,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Build chunks and log counts without calling Gemini or Supabase",
    )
...
def main() -> int:
    ...
    try:
        ...
        logger.exception("Population failed: %s", exc)
        return 1
